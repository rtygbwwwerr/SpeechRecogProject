...
int TrainTarget(Config& config)
{
	...
	try{
		...
		{             // linep gives the XLine with the Id of a given client and the list of files
			String *id=linep->getElement();                              // Get the Client ID (id)
			String *featureFileListName=linep->getElements().getElement();	           // Get the list of feature file for the client (end of the line)
			//	    String *featureFileListName=linep->getElement()/*.getElement()*/;	           // Get the list of feature file for the client (end of the line)
			...
			//-------------------------add name to featureFileListp-----------------
			ifstream input;
			input.open((*featureFileListName).c_str());
			if(!input)
			{
				cout<<"cannot open feature names!"<<endl;
				exit(1);
			}
			cout<<"open file_in OK~~"<<endl;

			XLine featureFileListp;
//			featureFileListp.addElement((*featureFileListName));

			int filecount = 0;
			while(!input.eof())
			{
				char buf[MAXCHARPERLINE];
				input.getline(buf,MAXCHARPERLINE);
				int len = strlen(buf);
				if(len>0)
				{
					 buf[len] = '\0';
					 featureFileListp.addElement(buf);
					 //for testing
					 String tp = featureFileListp.getElement(filecount);
					 cout<<tp<<endl;
				}
				filecount++;
			}
			...
			if (initByClient)
			{                                                                   // During trainig data statistic estimation by EM,
			...
			else
			{
				...
				if (!outputAdaptParam) 
				{
					...
					/*String szMixtureFilePath =config.getParam_mixtureFilesPath() +(*id);*/
