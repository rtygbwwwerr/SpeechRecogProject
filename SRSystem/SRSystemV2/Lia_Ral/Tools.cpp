	...
	SegCluster& selectedSegments, DoubleVector & globalMean,DoubleVector &globalCov,  unsigned long maxDistribCount, TrainCfg &trainCfg, unsigned long deleteDistrib){ 
		...
		try{
			...
			while((2*world.getDistribCount()) <= maxDistribCount)
			{
				...
				for (unsigned long trainIt=0; trainIt<trainCfg.getNbTrainIt(); trainIt++)
				{                           // Begin the initial EM iteration loop 
			...
			while(world.getDistribCount() < maxDistribCount)
			{
				...
				{                           // Begin the initial EM iteration loop 
					...
					varianceCeiling  = setItParameter(trainCfg.getInitVarCeil(),trainCfg.getFinalVarCeil(), trainCfg.getNbTrainIt(), trainIt);
					if (verboseLevel>2) 
						cout << "Train it["<<trainIt<<"], Variance floor["<<varianceFlooring<<"] ceiling["<<varianceCeiling<<"]"<<endl; 
					...
					if (trainCfg.getNormalizeModel()) normalizeMixture(world,config);	
					if (verbose) 
						cout << "Partial Train it["<<trainIt<<"] (take care, it corresponds to the previous it,0 means init likelihood) ="<<llkPreviousIt<<" Nb Frames="<<emAcc.getEMFeatureCount()<<endl;
					if (verboseLevel>2) 
						cout << "Train it["<<trainIt <<"] Complete LLK=[" << meanLikelihood(ss,fs,world,selectedSegments,config)<< "]"<<endl;       
					ss.deleteMixtureStat(emAcc);
				}
				if (verboseLevel>1)
					cout << " Final ll world = " << meanLikelihood(ss,fs,world,selectedSegments,config)<< endl;
				if(verbose && verboseLevel > 2)
				{
					cout << world.toString() << endl;
					for(unsigned long d=0; d<world.getDistribCount(); d++)
			...
			if(verbose) 
				cout << "Final Gaussian Nb: " << world.getDistribCount() << endl;
